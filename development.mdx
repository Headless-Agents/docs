...
```bash pip
pip install headless-agents
```

```bash poetry
poetry add headless-agents
```
...
```bash npm
npm install @headless-agents/sdk
```

```bash yarn
yarn add @headless-agents/sdk
```
...
```python
from headless_agents import HeadlessAgents
...
```typescript
import { HeadlessAgents } from '@headless-agents/sdk';
